...
<p align="center">
  <strong>An enterprise-grade, full-stack Human Resource Management & Automated Payroll Engine built with modern pastel aesthetics, dynamic mathematical formula evaluation, multi-level leave approvals, real-time shift punch tracking, automated payslip PDF generation, and Gmail/SMTP email delivery.</strong>
</p>
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Email-Nodemailer%20%7C%20Gmail%20SMTP-ea4335?style=flat-square" alt="Email" />
...
- **Multi-Department Kanban Board**: Visual workforce management board with full CRUD operations, department filters, and drag/switch assignment.
- **Large-Scale Data Handling**: Scalable data architecture supporting 200–500+ employee records with search, filter pills, and configurable pagination (10, 25, 50, 100, 250, 500 rows per page).
...
- **Working Schedules**: Multi-day shift configuration (start time, end time, scheduled work days) with **auto-calculated weekly hours** linked to employment agreements.
...
- **Quick Clock-In / Clock-Out**: Single-click shift punching from the global navigation topbar with live pulse indicator and active shift timer.
- **Automatic Shift Duration & Hours Worked**: Computes exact hours worked on the fly (`workedHours`), overtime tracking, and punctuality status flags (`PRESENT`, `LATE`, `HALF_DAY`, `ABSENT`).
- **Audit & Manual Corrections**: HR staff can record manual entries and adjust daily attendance logs.
...
- **5-Stage State Machine**: `DRAFT` ➔ `CALCULATING` ➔ `CALCULATED` ➔ `APPROVED` ➔ `FINALIZED` ➔ `PAID`.
...
### 8. 📄 Automated PDF Payslips & ✉️ Email Delivery
- **Automated Email Dispatch**: Marking a payrun as `PAID` or clicking `Send Payslips` automatically compiles each employee's official **Payslip PDF statement** and delivers it directly to their email via **Nodemailer (Gmail SMTP)**.
- **Itemized Statements**: Includes reference code (`PS-YYYY-MM-XXXX`), employee dossier, itemized earnings breakdown, deductions breakdown, net payout highlight, and company authorization.
...
- **Strict Data Isolation**: Dedicated portal for staff to view their personal salary statements, attendance logs, and leave balances with zero cross-employee exposure.
- **Self-Service Punching**: Clock in and clock out with live shift status and history.
- **Document Access**: One-click download of personal PDF payslips anytime.
...
│   │   │   ├── payroll.service.js        # Batch Payroll Computation & State Machine
│   │   │   ├── email.service.js          # Nodemailer Gmail SMTP Dispatcher
│   │   │   ├── pdf.service.js            # PDFKit In-Memory Statement Generator
│   │   │   ├── salaryRuleEngine.service.js # Math.js Formula Evaluator
│   │   │   ├── attendance.service.js     # Punch Logs & Worked Hours Engine
│   │   │   └── employee.service.js       # Workforce CRUD & Code Generator
...
| **Server** | Node.js (v18+), Express 5, Prisma ORM 6, Nodemailer, PDFKit, Math.js, Bcrypt.js, Cookie Parser |
| **Database** | MySQL 8.0+ |
| **Email Delivery** | Nodemailer with Gmail SMTP (`smtp.gmail.com:465`) & Ethereal Test Inboxes |
...
| **`PAYROLL_MANAGER`**| Create salary structures, define mathematical rules, trigger payroll computations, approve batches, generate & email payslips. |
...
# JWT Secrets
JWT_SECRET="peoplepay360-jwt-secret-change-in-production"
JWT_REFRESH_SECRET="peoplepay360-refresh-secret-change-in-production"
JWT_EXPIRES_IN="15m"
...
- `GET    /api/employees` — List employees with search, department filters, pagination (up to 500/page).
...
- `GET  /api/attendance` — Attendance history with worked hours and punctuality tags.
...
- `POST /api/payroll/payruns/:id/mark-paid` — Mark batch as paid, finalize disbursement, and automatically email payslip PDFs.
- `POST /api/payroll/payruns/:id/send-payslips` — Trigger bulk email delivery of payslip PDFs with attachments.

### Payslips (`/api/payslips`)
- `GET /api/payslips` — List payslips with generated reference numbers (`PS-YYYY-MM-XXXX`).
